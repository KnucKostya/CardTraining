...
const getCards = createAppAsyncThunk<GetCardsResponse, { packId: string, page:number,pageCount:number,cardName: string}>(
  "cards/get",
  async (arg, thunkAPI) => {
    return thunkTryCatch(
      thunkAPI,
      async () => {
        const res = await cardsApi.getCards(arg.packId,arg.page,arg.pageCount);
